  ...
  endpoints: (builder) => ({
    getPackagees: builder.query({
      ...
      providesTags: (result, error, arg) => {
        if (result?.ids) {
          return [{ type: "Packagee", id: "LIST" }, ...result.ids.map((id) => ({ type: "Packagee", id }))];
    ...
    addNewPackagee: builder.mutation({
      ...
      providesTags: (result) => {
        return result ? [...result.items.map(({ id }) => ({ type: "Packagee", id })), { type: "Packagee", id: "LIST" }] : [{ type: "Packagee", id: "LIST" }];
      },
      invalidatesTags: [{ type: "Packagee", id: "LIST" }],
    }),
    updatePackagee: builder.mutation({
      ...
      invalidatesTags: (result, error, arg) => [{ type: "Packagee", id: arg.id }],
    }),
...
export const { useGetPackageesQuery, useAddNewPackageeMutation, useUpdatePackageeMutation, useDeletePackageeMutation, useSendPackageeMutation } =
  packageesApiSlice;
